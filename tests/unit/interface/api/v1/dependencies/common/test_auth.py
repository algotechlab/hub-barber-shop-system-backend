...
@pytest.mark.asyncio
async def test_employee_or_user_sets_employee_state_when_employee_token():
    employee_id = uuid4()
    company_id = uuid4()
    request = Request({
        'type': 'http',
    ...
    mock_employee_repo = MagicMock()
    mock_employee_repo.get_employee = AsyncMock(return_value=MagicMock())
    settings = MagicMock(JWT_SECRET='secret', JWT_ALGORITHM='HS256')
    ...
    with (
        ...
        result = await require_current_employee_or_user(
            request=request,
            session=AsyncMock(),
            authorization='Bearer token',
        )
...
@pytest.mark.asyncio
async def test_require_current_employee_or_user_sets_user_state_when_user_token():
    user_id = uuid4()
    company_id = uuid4()
    request = Request({
        'type': 'http',
...
@pytest.mark.asyncio
async def test_require_current_employee_or_user_raises_when_invalid_token_type():
    request = Request({
        'type': 'http',
        'headers': [],
        'method': 'GET',
    ...
    with (
        ...
        patch(
            'src.interface.api.v1.dependencies.common.auth.jwt.decode',
            return_value={'sub': str(uuid4()), 'typ': 'owner'},
        ),
    ):
        with pytest.raises(UnauthorizedException, match='Token inválido'):
...
@pytest.mark.asyncio
async def test_require_current_employee_or_user_raises_when_employee_invalid():
    employee_id = uuid4()
    company_id = uuid4()
    request = Request({
        'type': 'http',
...
@pytest.mark.asyncio
async def test_require_current_employee_or_user_raises_when_user_invalid():
    user_id = uuid4()
    company_id = uuid4()
    request = Request({
        'type': 'http',

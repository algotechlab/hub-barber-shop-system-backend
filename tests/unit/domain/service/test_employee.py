...
class TestEmployeeService:
    ...
    async def test_list_employees_delegates_to_repository(
        ...
        result = await service.list_employees(pagination)

        mock_repository.list_employees.assert_awaited_once_with(pagination)
    ...
    async def test_get_employee_delegates_to_repository(
        ...
        mock_repository.get_employee.assert_awaited_once_with(employee_id)
        assert result == employee_out_dto
    ...
    async def test_update_employee_delegates_to_repository(
        ...
        result = await service.update_employee(employee_id, update_dto)

        mock_repository.update_employee.assert_awaited_once_with(
            employee_id, update_dto
        )
        assert result == employee_out_dto
    ...
    async def test_delete_employee_delegates_to_repository(
        ...
        mock_repository.delete_employee.assert_awaited_once_with(employee_id)

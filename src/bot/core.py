...
class BotCore:
    ...
    def _get_occupied_slots(self, period: str) -> list:
        ...
        try:
            ...
            if not employee_id:
                print(f"WARNING: No employee_id found for {self.sender_number}")
                return []
            
            employee_id = int(employee_id)
            occupied_slots = self.scheduler._get_occupied_slots(period, employee_id)
    ...
    def _generate_time_slots_message(self, period: str) -> str:
        ...
        self.session.set(f"{self.sender_number}_slots", json.dumps(available_slots))
        return (
    ...
    def _handle_time_slot_selection(self) -> str:
        ...
        scheduler_state = self.session.get(f"{self.sender_number}_scheduler_state")
        if not scheduler_state or ":" not in scheduler_state:
            print(f"ERROR: Invalid scheduler state for {self.sender_number}: {scheduler_state}")
            return self._reset_session()

        try:
            _, employee_id, product_id = scheduler_state.split(":")
            period = self.session.get(f"{self.sender_number}_selected_period") or ""
            if not period:
            ...
            slots_json = self.session.get(f"{self.sender_number}_slots") or "[]"
            available_slots = json.loads(slots_json)
            
            if slot_index < 0 or slot_index >= len(available_slots):
                print(f"WARNING: Invalid slot index {self.message} for {self.sender_number}")
                return "Opção inválida. Digite um número válido do horário."
            
            selected_time = available_slots[slot_index]
            
            self.session.set(self.sender_number, f"awaiting_slot:{employee_id}:{product_id}:{period}")
            print(f"DEBUG: Set state to awaiting_slot:{employee_id}:{product_id}:{period}, selected time {selected_time} for {self.sender_number}")
            
            return self._continue_schedule_flow(period, selected_time)
        except ValueError:
            print(f"WARNING: Non-numeric input for slot selection: {self.message}")
            return "Por favor, digite apenas o número do horário desejado."
        except json.JSONDecodeError as e:
            print(f"ERROR: Failed to decode slots JSON for {self.sender_number}: {e}")
            return self._reset_session()
    ...
    def _handle_period_selection(self) -> str:
        ...
        self.session.set(f"{self.sender_number}_selected_period", selected_period)
        self.session.set(self.sender_number, "awaiting_time_slot")
        print(f"DEBUG: Set state to awaiting_time_slot, period {selected_period} for {self.sender_number}")
    ...
    def get_response(self) -> str:
        try:
            ...
            if state and state.startswith("awaiting_slot"):
                print(f"DEBUG: Forcing session reset due to awaiting_slot state {state} for {self.sender_number}")
                self._reset_session()
                state = None
            ...
            if self.message == "1":
                ...
                self.session.set(self.sender_number, "undefined")
                print(f"DEBUG: Set state to undefined for {self.sender_number}")
                ...
                print(f"DEBUG: User requested operating hours for {self.sender_number}")
                return (
                ...
                print(f"DEBUG: User requested support for {self.sender_number}")
                return (
                ...
                print(f"DEBUG: User requested their schedules for {self.sender_number}")
                return self.scheduler.list_schedules()

            print(f"DEBUG: No valid option selected, resetting to default for {self.sender_number}")
            return self._reset_session()
        except Exception as e:
    ...
    def _continue_schedule_flow(self, period: str, selected_time: str) -> str:
        ...
        if not state or ":" not in state:
            print(f"ERROR: Invalid scheduler state for {self.sender_number}: {state}")
            return self._reset_session()
        ...
        try:
            ...
            result = self.scheduler.register_schedule(user_id, product_id, employee_id, datetime_obj)
            print(f"DEBUG: Register schedule result for {self.sender_number}: {result}")
            return result
        except Exception as e:
            print(f"ERROR: Failed to register schedule for {self.sender_number}: {e}")
            return "Erro ao registrar agendamento. Tente novamente.\n\n" + RESPONSE_DICTIONARY["default"]

    def identify_user(self):
        try:
            stmt = select(User.id).where(User.phone == self.sender_number)
            result = db.session.execute(stmt).fetchone()
            print(f"DEBUG: User lookup for {self.sender_number}: {'Found' if result else 'Not found'}")
    ...
    def send_message(self):
        try:
            ...
            print(f"DEBUG: Sent message to {self.sender_number}: {response.status_code}, {response.text}")
            if response.status_code != 201:
                print(f"ERROR: Failed to send message to {self.sender_number}: {response.status_code}, {response.text}")
            return response
        except Exception as e:

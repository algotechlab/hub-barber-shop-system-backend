                ...
                "Data",
                {
                    "key": fields.Nested(
                        webhook_ns.model(
                            "Key",
                            {
                                "remoteJid": fields.String(description="Sender phone"),
                                "fromMe": fields.Boolean(description="Message from bot"),
                                "id": fields.String(description="Message ID"),
                            }
                        )
                    ),
                    "pushName": fields.String(description="Sender's push name"),
                    ...
                    "messageTimestamp": fields.Integer(description="Timestamp"),
    ...
    def post(self):
        ...
        try:
            ...
            if payload_data.get("key", {}).get("fromMe", False):
                logdb("info", message="Ignoring message from bot (fromMe: True)")
                return {"status": "ignored", "message": "Message from bot"}, 200

            # Extrai número de telefone, ID da mensagem e timestamp
            phone_number = payload_data.get("key", {}).get("remoteJid", "").split("@")[0]
            ...
            if not phone_number or not message_id or not message_timestamp:
                logdb("error", message=f"Missing phone number, message ID, or timestamp: from={phone_number}, id={message_id}, timestamp={message_timestamp}")
            ...
            if session.client.get(dedup_key):
                logdb("info", message=f"Duplicate message ignored: {message_id}, timestamp: {message_timestamp}")
                return {"status": "ignored", "message": "Duplicate message"}, 200
            session.client.set(dedup_key, "processed", ex=600)  # Expira em 10 minutos
            ...
            if rate_count > 5:
                logdb("warning", message=f"Rate limit exceeded for {phone_number}")
            ...
            if not message_text:
                logdb("error", message=f"No message text found: from={phone_number}")
            ...
            #     logdb("info", message=f"Response sent to {phone_number}: {response.text}")
            #     return {"status": "success"}, 200
            # logdb("error", message=f"Failed to send response to {phone_number}: {response.text if response else 'No response'}")
            # return {"status": "error", "message": "Failed to send response"}, 200

        except Exception as e:
            print("coletnado o erro 500", e)
            logdb("error", message=f"Error processing webhook: {str(e)}")
            return {"status": "error", "message": "Error processing webhook"}, 500

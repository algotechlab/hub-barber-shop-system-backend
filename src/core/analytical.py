...
class AnalyticalCore:
    ...
    def summary_client(self, user_id: int):
        try:
            summary = select(
                self.user.username, self.user.lastname, self.user.phone
            ...
            total_spent_stmt = (
                ...
                .join(
                    self.products,
                    self.products.id == self.schedule_service.product_id,
                )
                .where(
                    self.schedule_service.is_check == True,
                    self.schedule_service.user_id == user_id,
            ...
            barbers_stmt = (
                ...
                .join(
                    self.employee,
                    self.employee.id == self.schedule_service.employee_id,
                )
                .where(
                    self.schedule_service.is_check == True,
                    self.schedule_service.user_id == user_id,

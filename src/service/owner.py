...
class OwnerService:
    ...
    def add_owner(self, owner_data: dict) -> tuple:
        try:
            ...
            return ApiResponse(
                success=True, message="Owner added successfully.", status_code=201
            ).to_response()
        except IntegrityError:
            self.db_session.rollback()
            return ApiResponse(
                success=False, message="Owner with this email already exists.", status_code=409
            ).to_response()
        except Exception:
            self.db_session.rollback()
            return ApiResponse(
                success=False, message="Error occurred while adding owner.", status_code=500
            ).to_response()
    ...
    def list_owners(self, data: dict) -> ApiResponse:
        try:
            ...
            return ApiResponse(
                status_code=500, message="Error processing list owners", error=True
            ).to_response()
    ...
    def get_owner(self, owner_id: int) -> ApiResponse:
        try:
            ...
            return ApiResponse(
                status_code=500, message="Error processing get owner", error=True
            ).to_response()
    ...
    def update_owner(self, owner_id: int, update_data: dict) -> tuple:
        try:
            ...
            for key, value in update_data.items():
                if value is not None and key in OWNER_FIELDS and hasattr(self.model, key):
            ...
            stmt = update(self.model).where(self.model.id == owner_id).values(**filtered_update)
    ...
    def delete_owner(self, owner_id: int) -> ApiResponse:
        try:
            ...
            self.db_session.rollback()
            return ApiResponse(
                status_code=500, message="Error processing delete owner", error=True
            ).to_response()

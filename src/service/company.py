...
class CompanyService:
    ...
    def add_company(self, data: dict) -> ApiResponse:
        try:
            ...
            return ApiResponse(
                status_code=400,
                message_id="company_already_exists",
                error=True,
            ).to_response()
        except Exception:
            self.db_session.rollback()
    ...
    def list_companies(self, data: dict) -> ApiResponse:
        try:
            ...
            if pagination_params.filter_by:
                ...
                try:
                    stmt = stmt.filter(
                        func.unaccent(self.model.name).ilike(
            ...
            return ApiResponse(
                status_code=500,
                message="Error processing list companies",
                error=True,
            ).to_response()
    ...
    def delete_company(self, company_id: int) -> ApiResponse:
        try:
            ...
            return ApiResponse(
                status_code=500,
                message="Error processing delete company",
                error=True,
            ).to_response()

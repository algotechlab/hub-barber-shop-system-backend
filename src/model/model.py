...
class Employee(db.Model):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    username: Mapped[str] = mapped_column(db.String(120), nullable=False)
    cpf: Mapped[str] = mapped_column(db.String(20), unique=True, nullable=False)
    ...
    deleted_at: Mapped[datetime] = mapped_column(db.DateTime, nullable=True, server_default=func.now())
    is_deleted: Mapped[bool] = mapped_column(db.Boolean, default=False)
    

    def __repr__(self):
        return f"""{self.name} created employeesuccessfully"""